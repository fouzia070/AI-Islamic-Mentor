async function sendQuestion() {
    ...
    try {
        ...
        if (data.answer) {
            ...
            // ── Refresh sidebar history after new answer ──
            if (window._currentUserEmail) {
                fetch("/get-history", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ email: window._currentUserEmail })
                }).then(r => r.json()).then(function(histData) {
                    const items = histData.history ? [...histData.history].reverse() : [];
                    const evt   = new CustomEvent("refreshSidebar", { detail: items });
                    document.dispatchEvent(evt);
                }).catch(function(e) {
                    console.log("Sidebar refresh error:", e);
                });
            }
